...
TEST(client_adapter_pi_registers_every_registry_tool) {
    ...
    /* Ownership markers so install can rewrite its own block and leave a
     * user-authored file alone. */
    ASSERT_NOT_NULL(strstr(js, CBM_ADAPTER_MARKER_START));
    ASSERT_NOT_NULL(strstr(js, CBM_ADAPTER_MARKER_END));
    free(js);
    PASS();
}
